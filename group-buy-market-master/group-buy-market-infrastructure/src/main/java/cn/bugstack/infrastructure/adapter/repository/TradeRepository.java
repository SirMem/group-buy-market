...
import cn.bugstack.infrastructure.utils.dcc.DCCService;
...
public class TradeRepository implements ITradeRepository {
    ...
    @Resource
    private IInventoryReservationDao inventoryReservationDao;
    ...
    public MarketPayOrderEntity lockMarketPayOrder(GroupBuyOrderAggregate groupBuyOrderAggregate) {
        ...
        if (StringUtils.isBlank(teamId)) {
            ...
            eventPublisher.delayOrderPublish("过期消息", 600);
    ...
    public List<UserGroupBuyOrderDetailEntity> queryTimeoutUnpaidOrderList() {
        ...
        for (GroupBuyOrderList groupBuyOrderList : groupBuyOrderLists) {
            ...
            UserGroupBuyOrderDetailEntity userGroupBuyOrderDetailEntity = UserGroupBuyOrderDetailEntity.builder()
                    ...
                    .teamId(groupBuyOrder.getTeamId())
                    .activityId(groupBuyOrder.getActivityId())
                    .goodsId(groupBuyOrderList.getGoodsId())
                    .targetCount(groupBuyOrder.getTargetCount())
                    .completeCount(groupBuyOrder.getCompleteCount())
                    .lockCount(groupBuyOrder.getLockCount())
                    .validStartTime(groupBuyOrder.getValidStartTime())
    ...
    @Override
    public String reserveTotalStockTry(InventoryReservationEntity inventoryReservationEntity, int ttlMinutes) {
        String availKey = InventoryKeys.availKey(inventoryReservationEntity.getGoodsId());
        String resvKey = InventoryKeys.resvKey(inventoryReservationEntity.getOutTradeNo());

        // idempotent: already reserved for this order
        if (redisService.isExists(resvKey)) return inventoryReservationEntity.getOutTradeNo();

        long after = redisService.decr(availKey);
        if (after < 0) { // went below zero -> rollback and fail
            redisService.incr(availKey);
            return null;
        }

        String reservationSnapshot = JSON.toJSONString(inventoryReservationEntity);
        Boolean ok = redisService.setNx(resvKey, reservationSnapshot);
        if (!ok) { // failed to record reservation -> rollback counter
            redisService.incr(availKey);
            return null;
        } else {
            long ttlMillis = TimeUnit.MINUTES.toMillis(ttlMinutes);
            redisService.setValue(resvKey, reservationSnapshot, ttlMillis);
            eventPublisher.reversePublish(inventoryReservationEntity);
        }

        return inventoryReservationEntity.getOutTradeNo();
    }

    @Override
    public void confirmTotalStock(String outTradeNo) {
        String resvKey = InventoryKeys.resvKey(outTradeNo);
        if (!redisService.isExists(resvKey)) {
            log.warn("总库存确认失败，预占记录不存在 outTrade{}", outTradeNo);
            return;
        }

        String reservationSnapshot = redisService.getValue(resvKey);

        if (StringUtils.isBlank(reservationSnapshot)) {
            log.warn("总库存确认失败，预占记录为空 outTrade{}", outTradeNo);
            redisService.remove(resvKey);
            return;
        }

        InventoryReservationEntity inventoryReservationEntity = JSON.parseObject(reservationSnapshot, InventoryReservationEntity.class);

        eventPublisher.confirmPublish(inventoryReservationEntity);
    }

    @Override
    public void cancelTotalStock(String outTradeNo, String goodsId) {
        String resvKey = InventoryKeys.resvKey(outTradeNo);
        if (!redisService.isExists(resvKey)) {
            log.warn("总库存取消失败，预占记录不存在 outTrade{}", outTradeNo);
            return;
        }

        // 兜底查询缺失的商品信息
        if (StringUtils.isBlank(goodsId)) {
            GroupBuyOrderList groupBuyOrderList = groupBuyOrderListDao.queryGroupBuyOrderRecordByOutTradeNoOnly(outTradeNo);
        ...
        if (StringUtils.isBlank(goodsId)) {
            log.warn("总库存释放失败，缺少商品信息 outTradeNo:{} goodsId:{}", outTradeNo, goodsId);
            redisService.remove(resvKey);
            return;
        }
        ...
        redisService.remove(resvKey);
        redisService.incr(InventoryKeys.availKey(goodsId));
        eventPublisher.cancelPublish(inventoryReservationEntity);

    }

    @Override
    public void setTotalStock(String goodsId) {
        Long totalStock = activityInventoryDao.queryTotalStockByGoodsId(goodsId);
        String availKey = InventoryKeys.availKey(goodsId);
        redisService.setNx(availKey, totalStock);
    }

    @Override
    @Transactional
    public void updateTotalStockReserveCount(InventoryReservationEntity inventoryReservationEntity) {
        activityInventoryDao.updateAddReserveStockByGoodsId(inventoryReservationEntity.getGoodsId());
        insertStockFlowTable(inventoryReservationEntity);
    }

    @Override
    @Transactional
    public void confirmSkuTotalStockDB(InventoryReservationEntity inventoryReservationEntity) {

        //更新库存表
        activityInventoryDao.updateSettleStockByGoodsId(inventoryReservationEntity.getGoodsId());
        //插入流水表(确保幂等性)
        insertStockFlowTable(inventoryReservationEntity);
    }
    ...
    @Override
    @Transactional
    public void cancelTotalStockDB(InventoryReservationEntity inventoryReservationEntity) {
        activityInventoryDao.updateReleaseReserveByGoodsId(inventoryReservationEntity.getGoodsId());
        insertStockFlowTable(inventoryReservationEntity);
    }

    @Override
    @Transactional
    public void refundTotalStockDB(InventoryReservationEntity inventoryReservationEntity) {
        activityInventoryDao.updateRefundStockByGoodsId(inventoryReservationEntity.getGoodsId());
        insertStockFlowTable(inventoryReservationEntity);
    }

    private void insertStockFlowTable(InventoryReservationEntity inventoryReservationEntity) {
        try {
            InventoryReservation inventoryReservation = new InventoryReservation();
            BeanUtils.copyProperties(inventoryReservationEntity, inventoryReservation);
            inventoryReservation.setCreateTime(LocalDateTime.now());
            inventoryReservation.setUpdateTime(LocalDateTime.now());

            inventoryReservationDao.insert(inventoryReservation);

        } catch (DuplicateKeyException e) {
            log.warn("库存已经确认success");
