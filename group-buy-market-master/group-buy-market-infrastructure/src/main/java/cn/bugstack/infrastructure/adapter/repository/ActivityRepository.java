...
public class ActivityRepository extends AbstractRepository implements IActivityRepository {
    ...
    @Override
    public boolean insertGroupBuyActivity(GroupBuyActivityEntity activityEntity) {
        GroupBuyActivity activity = GroupBuyActivity.builder()
                .activityId(activityEntity.getActivityId())
                .activityName(activityEntity.getActivityName())
                .discountId(activityEntity.getDiscountId())
                .groupType(activityEntity.getGroupType())
                .takeLimitCount(activityEntity.getTakeLimitCount())
                .target(activityEntity.getTarget())
                ...
                .tagId(activityEntity.getTagId())
                .tagScope(activityEntity.getTagScope())
                .createTime(activityEntity.getCreateTime())
                .updateTime(activityEntity.getUpdateTime())
                .build();
        return groupBuyActivityDao.insert(activity) > 0;
    }

    @Override
    public boolean updateGroupBuyActivity(GroupBuyActivityEntity activityEntity) {
        int count = groupBuyActivityDao.updateByActivityId(
                activityEntity.getActivityId(),
                activityEntity.getActivityName(),
                activityEntity.getDiscountId(),
                activityEntity.getGroupType(),
                activityEntity.getTakeLimitCount(),
                activityEntity.getTarget(),
                activityEntity.getValidTime(),
                activityEntity.getStatus(),
                activityEntity.getStartTime(),
                activityEntity.getEndTime(),
                activityEntity.getTagId(),
                activityEntity.getTagScope()
        );
        if (count > 0) {
            redisService.remove(GroupBuyActivity.cacheRedisKey(activityEntity.getActivityId()));
            return true;
        }
        return false;
    }
    ...
    @Override
    public boolean deleteGroupBuyActivity(Long activityId) {
        int count = groupBuyActivityDao.deleteByActivityId(activityId);
        if (count > 0) {
    ...
    public GroupBuyActivityEntity queryGroupBuyActivityByActivityId(Long activityId) {
        ...
        return convertToEntity(activity);
    }

    @Override
    public List<GroupBuyActivityEntity> queryGroupBuyActivityList() {
        List<GroupBuyActivity> list = groupBuyActivityDao.queryGroupBuyActivityList();
    ...
    @Override
    public boolean bindSkuActivity(String source, String channel, String goodsId, Long activityId) {
        return skuActivityDao.upsert(source, channel, goodsId, activityId) > 0;
    }

    @Override
    public boolean unbindSkuActivity(String source, String channel, String goodsId) {
        return skuActivityDao.deleteBySCGoodsId(source, channel, goodsId) > 0;
    }

    @Override
    public SCSkuActivityEntity querySCSkuActivity(String source, String channel, String goodsId) {
        SCSkuActivity activity = skuActivityDao.querySCSkuActivityBySCGoodsId(SCSkuActivity.builder()
                .source(source)
                .channel(channel)
                .goodsId(goodsId)
                .build());
    ...
    private GroupBuyActivityEntity convertToEntity(GroupBuyActivity activity) {
        if (null == activity) return null;
